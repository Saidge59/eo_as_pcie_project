...
int main()
{
    try
    {
        ...
        {
            unsigned int tmp_bar;
            unsigned long long tmp_offset;
            unsigned int tmp_value;

            if (iss >> tmp_bar >> std::hex >> tmp_offset >> std::dec >> tmp_value)
            {
                uint32_t bar = static_cast<uint32_t>(tmp_bar);
                uint64_t offset = static_cast<uint64_t>(tmp_offset);
                uint32_t value = static_cast<uint32_t>(tmp_value);
        ...
        else if (cmd == "read_register")
        {
            ...
            if (iss >> tmp_bar >> std::hex >> tmp_offset)
            {
                uint32_t bar = static_cast<uint32_t>(tmp_bar);
                uint64_t offset = static_cast<uint64_t>(tmp_offset);
